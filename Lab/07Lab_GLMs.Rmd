...
ggplot(EPAair, aes(Ozone, fill = Year)) + geom_density(alpha = 0.5) + 
  labs(x = "Ozone AQI Value", y = "Density") + ggtitle("Ozone per Year")

#2
ggplot(EPAair, aes(Year, Ozone)) + geom_boxplot() + stat_summary()

#3
ggplot(EPAair, aes(Year, Ozone)) + geom_violin()
...
ggplot(EPAair, aes(Ozone, PM2.5)) + geom_point() + geom_smooth(method = lm) +
  geom_text(x = 100, y = 78, label=expression("PM2.5=0.38*Ozone+15.64"))
...
EPA_selected <- 
  EPAair %>%
  select(mean_Lat,mean_Lng,PM2.5,Ozone, Month, Year) %>%
  na.omit()
EPACorr <- cor(EPA_selected)
