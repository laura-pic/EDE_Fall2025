...
ggplot(PeterPaul.chem.nutrients, aes(month)) + geom_bar()
...
PP_plot <- PeterPaul.chem.nutrients %>%
  filter(depth == 0) %>%
ggplot(aes(daynum, temperature_C, color = year4)) + geom_point() +
  facet_wrap(vars(lakename), nrow = 2) + ylab("Temperature")
print(PP_plot)
...
PeterPaul.chem.nutrients %>%
  ggplot(aes(sampledate, temperature_C, colour = depth)) + 
  geom_point(size = 0.5) + 
...
EPAair %>%
  ggplot(aes(PM2.5, Ozone, color = mean_Lat)) + geom_point(alpha = 0.5) +
...
EPAair %>% 
  ggplot(aes(Year, PM2.5)) + geom_boxplot()

EPAair %>% 
  ggplot(aes(Year, PM2.5)) + geom_violin()


EPAair %>% 
  ggplot(aes(Year, PM2.5)) + geom_density()
