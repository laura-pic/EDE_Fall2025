...
NTL.phys.data.tuesday <- NTL.phys.data %>% 
  filter(lakename == "Tuesday Lake", 
  year4 %in% c(1990:1999)) %>%  mutate(month = month(sampledate)) %>%
  filter(month == 7)
...
NTL.phys.data.tuesday_surface <- NTL.phys.data.tuesday %>% 
  filter(depth == 0)

mean(NTL.phys.data.tuesday_surface$temperature_C)
...
NTL.phys.data.gathered <- pivot_longer(
  NTL.phys.data, irradianceWater:irradianceDeck, names_to = "irradianceType",
