...
ggplot(Temp_BR_processed, aes(Date, Temperature_C)) + geom_line() + 
  geom_smooth(method = lm, se = F)

ggplot(Rainfall_BR_processed, aes(Date, Rainfall)) + geom_line() + 
  geom_smooth(method = lm, se = F)
...
BR_rain_decom <- stl(rainfall_ts, s.window = "periodic")
plot(BR_rain_decom)

BR_temp_decom <- stl(temp_ts, s.window = "periodic")
plot(BR_temp_decom)
...
BR_rain_components <-
  as.data.frame(BR_rain_decom$time.series[,1:3])

BR_temp_components <-
  as.data.frame(BR_temp_decom$time.series[,1:3])
