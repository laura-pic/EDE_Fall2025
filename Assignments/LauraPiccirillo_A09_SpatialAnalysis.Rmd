...
> ANSWER: 4269. This is a geographic CRS as it uses latitude/longitude measurements (angular units). This datum is
>associated with the North American Datum 1983.
...
> ANSWER: dec_long_va (x coordinates) and dec_lat_va (y coordinates).
...
>ANSWER: A geometry field appeared in the sf dataframe of gage locations. dec_lat_va and dec_long_va disappeared in the new sf dataframe.
...
site_flow_data$station_nm <- as.factor(site_flow_data$station_nm)


#15. Show the structure of the dataframe 
glimpse(site_flow_data)


#16. Join the flow data to our NWIS gage location spatial dataframe
all_gage <- inner_join(gage_sf, site_flow_data, by = c("site_no"))
...
ggplot() + geom_sf(data=nebraska_counties, color = "gray") +
  geom_sf(data = all_gage, aes(color = gage_ht)) + 
  theme(legend.position = "top") + labs(color = "Gage Height (m)") + 
  ggtitle("NWIS Gage Locations in Nebraska")
...
nebraska_counties_2 <- nebraska_counties %>%
...
#21 Plot the data
ggplot() + geom_sf(data=nebraska_counties_2) + 
  geom_sf(data=gage_sf_2,color="blue")
...
washington <- nebraska_counties_2 %>%
  filter(NAME=="Washington")

#23 Spatially select gages within the selected county to a new spatial dataframe
washington_gages <- gage_sf_2 %>%
  st_filter(washington)

#24 Spatially select gages within 20k of the selected county to a new spatial dataframe
washington_20km <- st_buffer(washington,20000)
washington_20km_gages <- gage_sf_2 %>%
  st_filter(washington_20km)

#25 Plot
ggplot() + geom_sf(data = nebraska_counties_2) + 
  geom_sf(data = washington, fill = "navyblue", color = "black") + 
  geom_sf(data=washington_20km_gages, color = "orange") + 
  geom_sf(data=washington_gages, color = "red") + 
  ggtitle("Nebraska Counties & NWIS Gage Locations") +
  labs(subtitle = "Washington County + Gages within a 20km radius")
